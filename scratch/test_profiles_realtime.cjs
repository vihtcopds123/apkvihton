...
async function main() {
  ...
  userClient
    ...
    .on('postgres_changes', {
      ...
      console.log('REALTIME PROFILE UPDATE RECEIVED IN USER CLIENT:', payload);
    })
    .subscribe((status) => {
      console.log('Subscription status:', status);
    });

  // Let's also check with the adminClient to see if it receives the profile update
  adminClient
    ...
    .on('postgres_changes', {
      ...
      console.log('REALTIME PROFILE UPDATE RECEIVED IN ADMIN CLIENT:', payload);
    })
    .subscribe();
  ...
  setTimeout(async () => {
    ...
    const { data, error } = await adminClient
      .from('profiles')
      .update({ full_name: randomName })
      .eq('id', userA.id)
      .select();

    if (error) console.error('Profile update error:', error);
    else console.log('Profile updated to:', randomName);
  }, 4000);
